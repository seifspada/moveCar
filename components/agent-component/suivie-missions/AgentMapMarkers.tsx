...
function buildVehicleIcon(
  ...
  const svg = getVehicleIconSVG(status, selected, terminee, size);
  return divIcon({
    html: svg,
    className: "",
    iconSize: [size, size],
...
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 48 48">
  <defs>
    <filter id="sh${size}t${terminee ? 1 : 0}">
  ...
  <rect x="3" y="5" width="42" height="38" rx="10"
    ...
    filter="url(#sh${size}t${terminee ? 1 : 0})"/>
  ...
  useEffect(() => {
    ...
    async function loadRoadRoutes() {
      const entries = await Promise.all(missions.map(async (mission) => {
        ...
        const current: [number, number] = [mission.latitude, mission.longitude];
      ...
      setRoadRoutes((prev) => {
        const next = { ...prev };
        entries.forEach(([id, route]) => {
          if (route) next[id] = route;
          else delete next[id];
        });
        return next;
      });
  ...
  return (
    <>
      {missions.map((mission) => {
        ...
        const alreadyRated      = mission.noteAgent != null;
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <span style={{
                      ...
                      background: isTerminee ? "#c084fc"
                        : status === "normal" ? "#4ade80"
                        : status === "gps_old" ? "#fb923c"
                        : "#f87171",
                    }} />
                    {isTerminee ? "Mission terminée"
                  ...
                  <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: "8px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                      ...
                      <div style={{ background: "rgba(39,39,42,0.6)", borderRadius: "8px", padding: "6px 8px" }}>
                        ...
                        {mission.accuracy && (
                          <p style={{ fontSize: "9px", color: "#52525b", margin: "2px 0 0" }}>±{mission.accuracy}m</p>
                        )}
                      </div>
                    </div>
